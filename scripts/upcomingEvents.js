...
createCardsTemplate(upcomingEvents,template);
...
btnBuscador.onclick = () =>{
    ...
    createCardsTemplate(eventosFIltrado,template);
}

checksContainer.addEventListener('change', (event) => {
    let eventosFiltrados = doubleSearch(upcomingEvents);
    createCardsTemplate(eventosFiltrados,template);
});
